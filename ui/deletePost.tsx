...
const DeletePostContent:React.FC<Props> = 
({
toggleDelete, 
setToggleDelete, 
refresh, 
postID
   ...
   const  deletePost = async () => {
    ...
    try {
        setLoading(true);
        const res= await req.delete(`/api/Alhoda_Alnabawya/DeletePost/${postID}`);
    ...
    return (
      <>
        ...
        transition-all duration-200 fixed top-1/2 left-1/2 -translate-1/2 z-50 lg:w-96 w-11/12 max-w-sm m-auto bg-white dark:bg-main p-6 shadow-2xl rounded-4xl border border-main/5 dark:border-white/10`}>
          ...
          <h3 className="text-center text-main dark:text-white mb-6">هل انت متاكد من حذف المقال</h3>
